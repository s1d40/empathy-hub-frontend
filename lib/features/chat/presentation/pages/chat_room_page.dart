...
import 'package:empathy_hub_app/features/chat/presentation/widgets/chat_message_bubble_widget.dart';
import 'package:empathy_hub_app/features/chat/presentation/widgets/message_input_widget.dart';
...
class _ChatRoomPageState extends State<ChatRoomPage> {
  ...
  // initState is still fine for other initializations if needed,
  // but the scroll listener logic will move.
  // @override
  // void initState() {
  //   super.initState();
  // }
  ...
  @override
  void dispose() {
    // _scrollController.removeListener(_onScroll); // _onScroll was removed, so this is no longer needed
  ...
  String _getChatRoomTitle(BuildContext context, ChatRoom chatRoom) {
    ...
    if (currentUserState is Authenticated) {
      ...
      final otherParticipant = chatRoom.participants.firstWhere(
        ...
        orElse: () => chatRoom.participants.isNotEmpty ? chatRoom.participants.first : UserSimple(anonymousId: '', username: 'Unknown'),
      );
        ...
        body: Column(
          children: <Widget>[
            ...
            BlocBuilder<ChatRoomCubit, ChatRoomState>( // To get isSending status
              builder: (context, state) {
                bool isSending = state is ChatRoomSendingMessage;
